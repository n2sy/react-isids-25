...
function Login() {
    ...
    function submitHandler(e) {
        ...
        if(showRegister) {
            ...
            axios.post("http://localhost:3000/auth/login", user).then(res => {
  ...
  return (
    <form className={classes.form} onSubmit={submitHandler}>
        ...
        <div className={classes.actions}>
        { showRegister &&   <button onClick={() => setShowRegister(previous => !previous)}>Switch to Login</button> }
        </div>
        <div className={classes.actions}>
        { !showRegister &&   <button >Login</button> }
        </div>
        <div className={classes.actions}>
        { !showRegister &&    <button  onClick={() => setShowRegister(previous => !previous)}>Switch to Register</button> }
        </div>
